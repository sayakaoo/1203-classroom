...
`;
    ...
    prompt = `ユーザーは三重引用符で囲んだテキストを提供します。これは「正方形をn番目までつくる場合マッチ棒は何本必要か？」というマッチ棒の規則性問題の回答です。ユーザーは4+3nという回答の説明をしています。この式は誤答です。次のステップで手順を実行してください。

    """${apiUserAnswer}"""
    
    ステップ 1: ユーザーの回答「${apiUserAnswer}」を解析し、4+3nになる理由を適切に説明しているかを判断してください。
    ステップ 2: ステップ 1の判断に基づき、次のいずれかを出力してください。
    - ユーザーの回答が正解の場合: {"result": "正解", "reason": "<理由の簡潔な要約>"}
